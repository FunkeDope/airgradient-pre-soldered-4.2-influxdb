...
Updated for InfluxDB by @funkedope
*/
...
String APIURL = ""; // something like "http://10.1.10.20:8086/api/v2/write?org=airquality&bucket=airquality&precision=s";
String APITOKEN = ""; //influx API token with write access.
...
void sendToServer() {
   if (currentMillis - previoussendToServer >= sendToServerInterval) {
     previoussendToServer += sendToServerInterval;
      ...
      + (pm25 < 0 ? "" : ", \"pm25\":" + String(pm25))
      + (pm25AQI < 0 ? "" : ", \"pm25AQI\":" + String(pm25AQI))
...
void sendToServer() {
   if (currentMillis - previoussendToServer >= sendToServerInterval) {
     previoussendToServer += sendToServerInterval;
     String lineProtocol = "air_quality,location=deck,unit_id=AG1";
     lineProtocol += " wifi=" + String(WiFi.RSSI());
     if (Co2 >= 0) lineProtocol += ",rco2=" + String(Co2);
     if (pm25 >= 0) lineProtocol += ",pm25=" + String(pm25);
     if (pm25AQI >= 0) lineProtocol += ",pm25AQI=" + String(pm25AQI);
     if (TVOC >= 0) lineProtocol += ",tvoc_index=" + String(TVOC);
     if (NOX >= 0) lineProtocol += ",nox_index=" + String(NOX);
     lineProtocol += ",atmp=" + String(temp);
     if (hum >= 0) lineProtocol += ",rhum=" + String(hum);
